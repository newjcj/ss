...
class WxPayApi
{
	...
	/**
     *author jcj
     * 企业付款，WxPayOrderQuery中out_trade_no、transaction_id至少填一个
     * appid、mchid、spbill_create_ip、nonce_str不需要填入
     * @param WxPayOrderQuery $inputObj
     * @param int $timeOut
     * @throws WxPayException
     * @return 成功时返回，其他抛异常
     */
    public static function orderTransfers($inputObj, $timeOut = 6)
    {
...
//        }
        ...
        $inputObj->SetMch_id(WxPayConfig::MCHID);//商户号
        $inputObj->SetCheck_name('NO_CHECK');//校验用户姓名选项  NO_CHECK：不校验真实姓 FORCE_CHECK：强校验真实姓名
        $inputObj->SetNonce_str(self::getNonceStr());//随机字符串
...
//            curl_setopt($ch,CURLOPT_SSLKEYTYPE,'PEM');
            curl_setopt($ch, CURLOPT_CAINFO, WxPayConfig::CAINFO_PATH);// CA根证书（用来验证的网站证书是否是CA颁布）jcj
		}
		//post提交方式
